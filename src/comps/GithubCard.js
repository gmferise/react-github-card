      ...
      <Card.Body>
        {
          visible ? (
            userData.hasOwnProperty('message') ? (<>
              <Card.Title>{userData.message}</Card.Title>
            </>) : (<>
              <Card.Img className="profile-img border border-dark" variant="top" src={userData.avatar_url} />
        ...
        <div className="text-center">
          <Button className="mt-2 mb-1" variant="secondary" type="button" onClick={toggleVisibility}>
            {visible ? 'Hide' : 'Show'}
          </Button>
        </div>
      </Card.Body>
